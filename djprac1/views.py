...
def make_paginator(objects, page, num=3):
    ...
    try:
        ...
        object_list = paginator.page(1)  ##返回第一页
    except EmptyPage:
        object_list = paginator.page(paginator.num_pages)  ##返回最后一页
...
def pagination_data(paginator, page):
    ...
    if paginator.num_pages == 1:
        return {}     # 只有一页无法分页，无需显示分页导航条，不用任何分页导航条数据

    left = []  ##当前页左边连续的页码号，初始值为空
    right = [] ##当前页右边连续的页码好，初始值为空
    left_has_more = False #标示第1页页码后是否需要显示省略号
    right_has_more = False #标示第2页页码后是否需要显示省略号
    first = False #标示是否需要显示第1页页码号，因为如果当前页左边的连续页码号中已经含有第 1 页的页码号，此时就无需再显示第 1 页的页码号，其它情况下第一页的页码是始终需要显示的。初始值为 False
    last = False #标示是否需要显示最后一页的页码号。需要此指示变量的理由和上面相同。
...
def index(request):
    ...
    entries_all = models.Entry.objects.all()
    page = request.GET.get('page', 1)
    entry_list, paginator = make_paginator(entries_all, page)
